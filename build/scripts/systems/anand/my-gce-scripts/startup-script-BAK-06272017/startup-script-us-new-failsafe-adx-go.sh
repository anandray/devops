...
#Copy CRONJOBS:
sudo su - << EOF
gsutil cp gs://startup_scripts_us/scripts/cron_root-us-adx-go /var/spool/cron/root;
...
sudo gsutil cp gs://startup_scripts_us/scripts/startup-script-us-new-failsafe-adx-go.sh /root/scripts/;
...
# running go command
if netstat -apn | egrep ':::80|crosschannel' > /dev/null; then
...
/bin/sed -i 's/\*\/1 \* \* \* \* \/usr\/bin\/flock \-w 0 \/var\/run\/startup-script-us-new-failsafe-adx-go.lock \/bin\/sh \/root\/scripts\/startup-script-us-new-failsafe-adx-go.sh/\#\*\/1 \* \* \* \* \/usr\/bin\/flock \-w 0 \/var\/run\/startup-script-us-new-failsafe-adx-go.lock \/bin\/sh \/root\/scripts\/startup-script-us-new-failsafe-adx-go.sh/g' /var/spool/cron/root
