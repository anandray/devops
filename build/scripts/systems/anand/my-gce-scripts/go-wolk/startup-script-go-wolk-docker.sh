...
sudo gsutil -m cp gs://startup_scripts_us/scripts/go-wolk/swarm-bashrc-centos7 /root/.bashrc;
gsutil -m cp gs://startup_scripts_us/scripts/go-wolk/swarm-bashrc-centos7 /home/anand/.bashrc;
...
sudo gsutil cp gs://startup_scripts_us/scripts/go-wolk/go-wolk-docker-crontab /root/scripts/;
...
## GIT CLONE GO-WOLK phobos-bootnode branch:
if [ ! -d /var/www/vhosts/go-wolk ]; then
        echo "/var/www/vhosts/go-wolk does NOT exist, proceeding with git clone..."
        sudo su - << EOF
        mkdir -p /var/www/vhosts;
        cd /var/www/vhosts/;
	git clone --recurse-submodules -b phobos-bootnode git@github.com:wolkdb/go-wolk.git
        cd go-wolk
...
"
sudo docker pull wolkinc/go-wolk-geth;
sudo docker run --dns=8.8.8.8 --dns=8.8.4.4 --name=go-wolk-geth --rm -dit -p 30303:30303  -p 30303:30303/udp -p 30304:30304/udp wolkinc/go-wolk-geth
