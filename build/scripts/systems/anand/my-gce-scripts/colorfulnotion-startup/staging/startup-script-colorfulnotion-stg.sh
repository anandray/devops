...
if [ ! -d /home/goracing/go_projects/src/goracing.colorfulnotion.com ]; then
	...
	git clone --recurse-submodules -b staging git@github.com:colorfulnotion/goracing.colorfulnotion.com.git
...
sudo gsutil -m cp gs://startup-scripts-colorfulnotion/scripts/goracing-stg-*start.sh /root/scripts
sudo gsutil -m cp gs://startup-scripts-colorfulnotion/scripts/crontab-stg /var/spool/cron/root
...
# start goracing staging
#sh /root/scripts/goracing-stg-start.sh
