...
echo "php54 is installed"
/bin/sed -i 's/\*\/1 \* \* \* \* \/usr\/bin\/flock \-w 0 \/var\/run\/startup-script-us-new_failsafe.lock \/bin\/sh \/root\/scripts\/startup-script-us-new_failsafe.sh/\#\*\/1 \* \* \* \* \/bin\/sh \/root\/scripts\/startup-script-us-new_failsafe.sh/g' /var/spool/cron/root
...
sudo su - << EOF
gsutil -m cp gs://startup_scripts_us/scripts/nagios/nrpechk.shroot/scripts/;
...
sudo gsutil -m cp gs://startup_scripts_us/scripts/hbase/hbasechk.sh /root/scripts;
