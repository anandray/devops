...
#prefix="$app-$node-$region-$provider"
prefix="wlk-$node-gc"
instance_template="$region-template"
instancetemplate_image="$prefix-image"
urlmap="$prefix"
lbname="$prefix-https"
https_lbname="$prefix-https"
globalip="$app-$node-$region-$provider-global-ip"
targetproxy="$prefix-target-proxy-$https_port"
regionalipname="$app-$node-$region-$provider-regional-ip"
healthcheck="$app-$node-$region-healthcheck"
portname="$app-$node-$port"
https_portname="$app-$node-$https_port"
...
if ! gsutil ls -l gs://wolk-scripts/scripts/cloudstore/google-credentials/google.json-$project 2> /dev/null; then
...
if ! gcloud compute --project=$project instances list 2> /dev/null; then
...
# stop cron on Consensus Instance to prevent wolk compilation before image creation
ssh $regional_ip service crond stop
ssh $regional_ip rm -rf /usr/local/wolk/*
...
gcloud compute --project=$project instances create $fixedinstance --zone=$primaryzone --machine-type=custom-2-4096 --address $regional_ip --subnet=default --network-tier=PREMIUM --metadata=startup-script-url=gs://wolk-scripts/scripts/cloudstore/cloudstore-git-update-gcp-consensus-new.sh,ssh-keys=root:ssh-rsa\ AAAAB3NzaC1yc2EAAAABIwAAAQEA0\+dWJfxKJKozhrHQ8Zn06CsIXg3nav5tBi5ojZUKjDrOI78P0BPwaIod48fj8er8Z/spyW/pQ5Ys/TAL739TWPMtTQwfsWvsD1B5chOVVWrb5BomcEqWzcg/u6vCUqYFfP8q2p5p46w5U41nO\+S1mO\+NjdWsNn4f2Gqg8xIXZs/BDb02\+AdBZ/DTwj12HAJHoBpUF7OBLVibJwDGX4xj1BQuYtg7\+iCeaW2aW0NDCifU5bPOCZGQ4gAWG7HLGoqEE\+EGvousqXIJ\+K58Ex/G\+21qgaMqLr4QL\+ZBkHGEZ6q72/ziz7Nz9Na3XpanUDYcdfC1ppXlydtxV8yXQgSfNQ==\ root$'\n'Tapas:ssh-rsa\ AAAAB3NzaC1yc2EAAAABJQAAAQEAoOsmmxsNw4dvzOKUks3F1wDhEL43L\+7hCGhkOhX1gI04pFiaktMXoyEklN9ue8ESDWtTTL2D9mwBvlysexTG7FhjAVXfOnWByjz5mXoEzOIrM4bcrKuhTHCUujTXFDnkVJaKo5czFsobs/F7TNBMsIq788x7GfI1ntwc27kCFvAjsVSkwoF/VTb4/O7SyyDVbyf6F6jvOjRZKO/6JVzqJyMN5S740xAfp42I/cqUdM8t\+MUpQh/1EgzBlUiG4WFEDrcbmL/TWuRTa91BcHc57ym7ddTsljzS0I5iU5y1t2GCE3dhrRj\+KysxMgc3GmH3saEAymYt8PAPDNSdHkqgiQ==\ Tapas$'\n'anand:ssh-rsa\ AAAAB3NzaC1yc2EAAAADAQABAAABAQDofwXiKi9VAXlXETZMX16aSu3xZRuwVBB8Jgd/JIT3wG0yfCSymSvSS1Cc8nMm3hwywL5IxwCiIDXxrHsIsBeTuyQWSMZRcNcbEOC4fQxyRNBi1Arqb3t5OPIEZD1Y57C42vV0Hpk08zfysveuww7vJD69inCyGhE8IB4CD6Hn0N2aDQjMp3wtvao6c9aZh9OqtCpyHX0W8EC27RiTOf\+kFy2XxGQ544nJo08g//6bwbZVTS5/Ta4OOjPu92pu40BwnQQyOpPz0FErEGujInFeHhSp3mF3/MFavn5PO8Ne8sZ3CAHg\+NIrQO7i94UYTpnM22E/Xjr3FhN7/qmjHAM/\ anand --maintenance-policy=MIGRATE --service-account=$serviceAccount --scopes=https://www.googleapis.com/auth/cloud-platform --tags=allowall,http-server,https-server --image=wolk-$region-image --image-project=$project --boot-disk-size=20GB --boot-disk-type=pd-standard --boot-disk-device-name=$fixedinstance
fi

# start crond
ssh $regional_ip service crond restart
...
if ! gcloud compute --project=$project instance-templates list 2> /dev/null; then
gcloud compute --project=$project instance-templates create $instance_template --machine-type=custom-2-4096 --network=projects/$project/global/networks/default --network-tier=PREMIUM --metadata=startup-script-url=gs://wolk-scripts/scripts/cloudstore/cloudstore-git-update-gcp-storage-new.sh,ssh-keys=root:ssh-rsa\ AAAAB3NzaC1yc2EAAAABIwAAAQEA0\+dWJfxKJKozhrHQ8Zn06CsIXg3nav5tBi5ojZUKjDrOI78P0BPwaIod48fj8er8Z/spyW/pQ5Ys/TAL739TWPMtTQwfsWvsD1B5chOVVWrb5BomcEqWzcg/u6vCUqYFfP8q2p5p46w5U41nO\+S1mO\+NjdWsNn4f2Gqg8xIXZs/BDb02\+AdBZ/DTwj12HAJHoBpUF7OBLVibJwDGX4xj1BQuYtg7\+iCeaW2aW0NDCifU5bPOCZGQ4gAWG7HLGoqEE\+EGvousqXIJ\+K58Ex/G\+21qgaMqLr4QL\+ZBkHGEZ6q72/ziz7Nz9Na3XpanUDYcdfC1ppXlydtxV8yXQgSfNQ==\ root$'\n'anand:ssh-rsa\ AAAAB3NzaC1yc2EAAAADAQABAAABAQDofwXiKi9VAXlXETZMX16aSu3xZRuwVBB8Jgd/JIT3wG0yfCSymSvSS1Cc8nMm3hwywL5IxwCiIDXxrHsIsBeTuyQWSMZRcNcbEOC4fQxyRNBi1Arqb3t5OPIEZD1Y57C42vV0Hpk08zfysveuww7vJD69inCyGhE8IB4CD6Hn0N2aDQjMp3wtvao6c9aZh9OqtCpyHX0W8EC27RiTOf\+kFy2XxGQ544nJo08g//6bwbZVTS5/Ta4OOjPu92pu40BwnQQyOpPz0FErEGujInFeHhSp3mF3/MFavn5PO8Ne8sZ3CAHg\+NIrQO7i94UYTpnM22E/Xjr3FhN7/qmjHAM/\ anand$'\n'tapas:ssh-rsa\ AAAAB3NzaC1yc2EAAAABJQAAAQEAwXK1uMe3EQBW/uaQP1EdQtMW65ELJFtQKZ7314fmB43IhAXmm8opWI6uylF4ELRz2fhZX/s2Pu/XIe3NXmzRAg9w5gNbMQWl8ezNOOxpNPsImbYiVGuQJ2ldFEp\+d80eToGrcAciTv1jP2YW91P9MJwZ5LdM0AvQuwBn89xYVCn5ASTTzXzuUwxh\+zAjZKbYYF6iCMVysm5PUYBbWcAS69X9E4LAZNffPFDiS\+hEwLQtpmCXYrVQ\+EnCUQihldhhDe4xfxHi8JHHLyzH80glE/HLer3ygmrs/FOTPKuOQZ9jhqT6mqWjRY8JSLhwyft9OzNdAOenRBEubDzRAnNqaw==\ tapas --maintenance-policy=MIGRATE --service-account=$serviceAccount --scopes=https://www.googleapis.com/auth/cloud-platform --tags=allowall,http-server,https-server --image=wolk-$region-image --image-project=$project --boot-disk-size=20GB --boot-disk-type=pd-standard --boot-disk-device-name=$instance_template --project=$project
fi

#Create Managed Instance Group
## gcloud compute instance-groups managed create
gcloud -q beta compute --project=$project instance-groups managed create $instance_group --base-instance-name=$instance_group --template=$instance_template --size=1 --zones=$zones --initial-delay=300
...
#### LOAD BALANCER ####
# add ssl certificate
if ! gcloud compute ssl-certificates list --project=$project 2> /dev/null; then
...
# 6 Firewall
if ! gcloud compute firewall-rules list --format json | grep allow-all 2> /dev/null; then
...
gsutil cp gs://wolk-scripts/scripts/cloudstore/make-wolk.sh /tmp/make-wolk.sh;
chmod +x /tmp/make-wolk.sh;
scp /tmp/make-wolk.sh $regional_ip:/root/scripts/;
ssh $regional_ip chmod +x /root/scripts/make-wolk.sh;
ssh $regional_ip sh /root/scripts/make-wolk.sh
fi

if ! ssh -q $regional_ip ls -lt /root/go/src/github.com/wolkdb/cloudstore/wolk/cloud/credentials/genesis.json; then
scp c0.wolk.com:/root/go/src/github.com/wolkdb/cloudstore/wolk/cloud/credentials/genesis.json /tmp/genesis.json;
scp /tmp/genesis.json $regional_ip:/root/go/src/github.com/wolkdb/cloudstore/wolk/cloud/credentials/genesis.json
ssh -q $regional_ip service wolk restart
fi

if ! ssh -q $regional_ip ps aux | grep "wolk --datadir" | grep -v grep; then
ssh -q $regional_ip systemctl reset-failed wolk.service;
ssh -q $regional_ip systemctl start wolk.service
fi
