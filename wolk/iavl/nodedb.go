...
func (ndb *nodeDB) GetNode(hash []byte) *Node {
	...
	// Doesn't exist, load.
	//dprint("[iavl_nodedb:GetNode] hash(%x) ndb.nodekey(hash)(%x)", hash, ndb.nodeKey(hash))
...
func (ndb *nodeDB) SaveNode(node *Node) {
	...
	node.printnode()
	dprint("[iavl_nodedb:SaveNode] saving node!\n oldnodekey(%x),\n node.hash(%x), \n chunkid(%x), \n chunkval(%x), \n key(%x), \n valhash(%x)", ndb.nodeKey(node.hash), node.hash, wolkcommon.Computehash(buf.Bytes()), buf.Bytes(), node.key, node.value)

	dprint("[iavl_nodedb:SaveNode] batch set! key(%x), val(%x)", ndb.nodeKey(node.hash), buf.Bytes())

	ndb.batch.Set(ndb.nodeKey(node.hash), buf.Bytes())
	//debug("BATCH SAVE %X %p\n", node.hash, node)
	//dprint("[iavl_nodedb:SaveNode] do the chunk IDs match?\n wolk(%x)\n tendermint(%x)", wolkcommon.Computehash(buf.Bytes()), node.hash)
...
func (ndb *nodeDB) updateLatestVersion(version int64) {
	dprint("[iavl_nodedb:updateLatestVersion] to (%d)", version)
...
func (ndb *nodeDB) getRoot(version int64) []byte {
	dprint("[iavl_nodedb:getRoot] getting ROOT (%x)", ndb.rootKey(version))
...
func (ndb *nodeDB) SaveRoot(root *Node, version int64) error {
	dprint("[iavl_nodedb:SaveRoot] SAVING root(%x)", root.hash)
	if len(root.hash) == 0 {
...
func (ndb *nodeDB) saveRoot(hash []byte, version int64) error {
	ndb.mtx.Lock()
	defer ndb.mtx.Unlock()
	dprint("[iavl_nodedb:saveRoot] hash(%x) version(%d)", hash, version)
	dprint("[iavl_nodedb:saveRoot] getLatestVerion gets(%v)", version)
	if version != ndb.getLatestVersion()+1 {
		dprint("[iavl_nodedb:saveRoot] version is wrong! check(%d)", ndb.getLatestVersion()+1)
		return fmt.Errorf("Must save consecutive versions. Expected %d, got %d", ndb.getLatestVersion()+1, version)
	}

	key := ndb.rootKey(version)
	dprint("[iavl_nodedb:saveRoot] **** BATCH SET key(%x) hash(%x)", key, hash)
	ndb.batch.Set(key, hash)
