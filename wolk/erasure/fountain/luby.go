...
func NewLubyCodec(sourceBlocks int, degreeCDF []int) Codec {
	return &lubyCodec{
...
func (c *lubyCodec) PickIndices(codeBlockIndex int64) []int {
	...
	d := pickDegree(c.degreeCDF, h)
	return sampleUniform(h, d, c.sourceBlocks)
}
...
func EncodeLTBlocks(message []byte, encodedBlockIDs []int64, c Codec) []LTBlock {
	...
	for i := range encodedBlockIDs {
		...
		// fmt.Printf("EncodeLTBlocks:PickIndices(%d) %v\n", encodedBlockIDs[i], indices)
...
func (d *lubyDecoder) AddBlocks(blocks []LTBlock) bool {
	for i := range blocks {
		...
		// fmt.Printf("AddBlocks - PickIndices(%d) %v\n", blocks[i].BlockCode, indices)
