...
func RobustSolitonDistribution(n int, m int, delta float64) []float64 {
	return robustSolitonDistribution(n, m, delta)
}
...
func SolitonDistribution(n int) []int {
	cdf := make([]float64, n+1)
	cdf[1] = 1 / float64(n)
	for i := 2; i < len(cdf); i++ {
		cdf[i] = cdf[i-1] + (1 / (float64(i) * float64(i-1)))
	}
	cdfInt64 := make([]int, n+1)
	for i, c := range cdf {
		cdfInt64[i] = int(c * 10000)
	}
	return cdfInt64
}
...
func onlineSolitonDistribution(eps float64) []int {
...
func pickDegree(cdfi []int, h *big.Int) int {
	// crand.Int(crand.Reader, big.NewInt(10000))
	ri := int(h.Mod(h, big.NewInt(10000)).Int64())
	di := sort.SearchInts(cdfi, ri)
	//fmt.Printf("pickDegree ri=%d di=%v [%v]\n", ri, di, cdfi)
	if cdfi[di] > ri {
		return di
	}
...
func sampleUniform(h *big.Int, num, max int) []int {
	if num >= max {
		picks := make([]int, max)
		for i := 0; i < max; i++ {
			picks[i] = i
	...
	t := new(big.Int)
	for i := 0; i < num; i++ {
		p := t.Mod(h, big.NewInt(int64(max))).Int64() // random.Intn(max)
		h = nextInt(h)
		for seen[p] {
			p = t.Mod(h, big.NewInt(int64(max))).Int64()
			h = nextInt(h)
		}
		picks[i] = int(p)
		seen[p] = true
...
func nextInt(i *big.Int) (o *big.Int) {
	return new(big.Int).SetBytes(blakeb(i.Bytes()))
}
