...
func TestOnlineSolitonDistribution(t *testing.T) {
	...
	if cdf[0] != 0 {
		t.Errorf("CDF[0] = %d, should be 0.0", cdf[0])
		t.Log("CDF=", cdf)
	}

	if cdf[len(cdf)-1] != 1 {
		t.Errorf("CDF[max] = %d, should be very nearly 1", cdf[len(cdf)-1])
		t.Log("CDF=", cdf)
	}
...
func TestPickDegree(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		d := pickDegree(cdf, big.NewInt(int64(i)))
		if d < 1 || d > len(cdf)-1 {
...
func TestSampleUniform(t *testing.T) {
	...
	for a, i := range sampleTests {
		h := new(big.Int).SetBytes(blakeb(wolkcommon.IntToByte(int64(a))))
		out := sampleUniform(h, i.num, i.length)
