...
func NewImmutableTree(cs ChunkStore, cacheSize int) *ImmutableTree {
	if cs == nil {
		// In-memory Tree.
...
func (t *ImmutableTree) hashWithCount() ([]byte, int64) {
	...
	hash, count, _ := t.root.hashWithCount()
	return hash, count
}

// Get returns the index and value of the specified key if it exists, or nil
// and the next index, if it doesn't.
func (t *ImmutableTree) Get(key []byte) (index int64, value []byte, storageBytes uint64) {
	if t.root == nil {
...
func (t *ImmutableTree) GetByIndex(index int64) (key []byte, value []byte, storageBytes uint64) {
	if t.root == nil {
...
func (t *ImmutableTree) Iterate(fn func(key []byte, value []byte) bool) (stopped bool) {
	...
	return t.root.traverse(t, true, func(node *Node) bool {
		if node.height == 0 {
			return fn(node.key, node.valhash)
		}
		return false
	})
}
...
// If either are nil, then it is open on that side (nil, nil is the same as Iterate)
func (t *ImmutableTree) IterateRangeInclusive(start, end []byte, ascending bool, fn func(key, value []byte) bool) (stopped bool) {
	if t.root == nil {
		return false
	}
