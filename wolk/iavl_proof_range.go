...
type RangeProof struct {
	...
	LeftPath   PathToLeaf      `json:"lp"`
	InnerNodes []PathToLeaf    `json:"in"`
	Leaves     []proofLeafNode `json:"lfs"`
...
func (proof *RangeProof) VerifyItem(key, value []byte) error {
	...
	leaves := proof.Leaves
	if proof == nil {
		log.Error("[iavl_proof_range:VerifyItem] proof is nil")
		return cmn.ErrorWrap(ErrInvalidProof, "proof is nil")
	}
	if !proof.rootVerified {
		log.Error("[iavl_proof_range:VerifyItem] must call Verify(root) first.")
	...
	if !bytes.Equal(leaves[i].ValHash, value) {
...
func (proof *RangeProof) verify(root []byte) (err error) {
	...
	if !bytes.Equal(rootHash, root) {
		log.Error("[iavl_proof_range:verify] roothash doesn't match!", "rootHash", hex.EncodeToString(rootHash), "root", hex.EncodeToString(root))
	...
	COMPUTEHASH = func(path PathToLeaf, rightmost bool) (hash []byte, treeEnd bool, done bool, err error) {
		...
		for len(path) > 0 {
			...
			if !bytes.Equal(derivedRoot, lpath.Right) {
				return nil, treeEnd, false, fmt.Errorf("[proof_range:_computeRootHash] INVALID ROOT: intermediate root hash %X doesn't match, got %X", lpath.Right, derivedRoot)
			}
			if done {
...
func (t *ImmutableTree) getRangeProof(keyStart, keyEnd []byte, limit int) (proof *RangeProof, keys, values [][]byte, err error) {
	...
	//dprint("[proof_range:getRangeProof] tree.root hash: (%x)", t.root.hash)
	...
	var leaves = []proofLeafNode{proofLeafNode{
		...
		StorageBytesTotal: left.storageBytesTotal,
		Key:               left.key,
		//ValueHash: tmhash.Sum(left.valhash),
...
func (tree *MutableTree) GetVersionedWithProof(key []byte) ([]byte, *RangeProof, error) {
	//if tree.versions[version] {
	t, err := tree.GetImmutable()
	if err != nil {
		return nil, nil, err
	}
...
// and limit.
func (tree *MutableTree) GetVersionedRangeWithProof(startKey, endKey []byte, limit int) (
