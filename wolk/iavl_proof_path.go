...
func (pl PathToLeaf) verify(leafHash []byte, root []byte) cmn.Error {
	...
	if !bytes.Equal(root, hash) {
		log.Error("[proof_path:pathtoleaf:verify] root != hash", "root", hex.EncodeToString(root), "hash", hex.EncodeToString(hash))
...
func (pl PathToLeaf) computeRootHash(leafHash []byte) []byte {
	hash := leafHash
	//dprint("[iavl_proof_path:computeRootHash] leafHash:\t%x", hash)
	for i := len(pl) - 1; i >= 0; i-- {
		pin := pl[i]
		//dprint("[iavl_proof_path:computeRootHash] %d:\t%x\t(%x)", i, hash, pin.Hash(hash))
		hash = pin.Hash(hash)
	}
	return hash
}
