...
#/usr/bin/updatedb
#enode=$(grep -s "self=enode:" `locate bootnode.log | head -n1` | awk '{print$6}' | cut -d "=" -f2 | cut -d "@" -f1)
#ip=$(grep -R 'addr=172.17.0.2' `locate geth.log`  | head -n1 | awk '{print$4}' | cut -d "=" -f2 | cut -d ":" -f1)
#fi

enode=$(grep -s "self=enode:" `find /mnt -name 'bootnode.log' | head -n1` | awk '{print$6}' | cut -d "=" -f2 | cut -d "@" -f1)
ip=$(grep -R 'addr=' `find /mnt -name geth.log`  | head -n1 | awk '{print$4}' | cut -d "=" -f2 | cut -d ":" -f1)

#enode=`cat /wolk/scripts/enode.txt`
#ip=`cat /wolk/scripts/ip.txt`
...
# obtain networkid from WolkMain:
networkid=$(grep 'network=' `find /mnt -name 'geth.log'` | head -n1 | awk '{print$NF}' | cut -d "=" -f2)
