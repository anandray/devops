...
// Chunk also serves as a request object; in case it is a retrieval request, Data is nil and Size is 0
...
func MakeHashFunc(hash string) SwarmHasher {
	switch hash {
	case "SHA256":
		return func() SwarmHash { return &HashWithLength{sha256.New()} }
	case "BMT":
		return func() SwarmHash {
			hasher := sha256.New
			hasherSize := hasher().Size()
			segmentCount := chunkDefaultSize / hasherSize
...
func GenerateRandomChunks(dataSize int64, count int) (chunks []*Chunk) {
	...
	if dataSize > chunkDefaultSize {
		dataSize = chunkDefaultSize
	}
...
type Putter interface {
	Put(context.Context, ChunkData, *sync.WaitGroup) (Reference, error)
...
func (v *ContentAddressValidator) Validate(addr Address, data []byte) bool {
	if l := len(data); l < 9 || l > chunkDefaultSize+8 {
		return false
	}
...
func ComputeDefaultHashes(smtTreeDepth int) (defaultHashes [][]byte) {
	defaultHashes = make([][]byte, smtTreeDepth)
	empty := make([]byte, 0)
	defaultHashes[0] = wolkcommon.Computehash(empty)
	for level := 1; level < smtTreeDepth; level++ {
...
const (
	RegionNA = byte(1) // Region2Byte("NA")
	RegionAS = byte(2) // Region2Byte("AS")
	RegionEU = byte(3) // Region2Byte("EU")
	RegionAP = byte(4) // Region2Byte("AP")
)

func RegionStringToByte(region string) byte {
	if strings.Compare(region, "NA") == 0 {
		return RegionNA
	}
	if strings.Compare(region, "EU") == 0 {
		return RegionEU
	}
	if strings.Compare(region, "AP") == 0 {
		return RegionAP
	}
	if strings.Compare(region, "AS") == 0 {
		return RegionAS
	}
	return RegionNA
}

func ipstring_to_netip(ip string) (netip net.IP) {
