...
const (
	chunkDefaultSize = 8192
	//chunkDefaultSize = 16384
	//chunkDefaultSize = 65586
	ChunkProcessors = 8
	splitTimeout    = time.Minute * 30
)
...
This chunker works in a simple way, it builds a tree out of the document so that each node either represents a chunk of real data or
 a chunk of data representing an branching non-leaf node of the tree.
 In particular each such non-leaf chunk will represent is a concatenation of the hash of its respective children.
This scheme simultaneously guarantees data integrity as well as self addressing.
Abstract nodes are transparent since their represented size component is strictly greater than their maximum data size,
since they encode a subtree.

If all is well it is possible to implement this by simply composing readers so that no extra allocation or buffering is necessary
for the data splitting and joining. This means that in principle there can be direct IO between : memory, file system, network socket
(bzz peers storage request is read from the socket). In practice there may be need for several stages of internal buffering.
...
func (tc *TreeChunker) Split(ctx context.Context) (k Address, wait func(context.Context) error, err error) {
	...
	go tc.split(depth, treeSize/tc.branches, key, tc.dataSize, tc.wg, tc.putterwg)
	...
	// TODO CHECK defer tc.putter.Close()
	select {
	case err := <-tc.errC:
		if err != nil {
			return nil, nil, err
...
func (tc *TreeChunker) split(depth int, treeSize int64, addr Address, size int64, parentWg *sync.WaitGroup, putterWg *sync.WaitGroup) {

	//fmt.Printf(" split(depth=%d, treesize=%d, Address=%s, size: %d)\t", depth, treeSize, addr, size)
	for depth > 0 && size < treeSize {
		treeSize /= tc.branches
		depth--
	}
	//fmt.Printf("   .. depth => %d  treeSize => %d\n", depth, treeSize)
	...
	if depth == 0 {
		...
		counter++
		select {
		case tc.jobC <- &hashJob{addr, chunkData, size, parentWg, putterWg}:
		case <-tc.quitC:
		}
		return
	}
	...
	childrenPutterWg := &sync.WaitGroup{}
	var secSize int64
	for i < branchCnt {
		...
		childrenWg.Add(1)
		tc.split(depth-1, treeSize/tc.branches, subTreeKey, secSize, childrenWg, childrenPutterWg)
	...
	select {
	case tc.jobC <- &hashJob{addr, chunk, size, parentWg, putterWg}:
	case <-tc.quitC:
	}
}
	...
	go func() {
		...
		for {
			...
			case job, ok := <-tc.jobC:
				...
				job.putterWg.Add(1)
				h, err := tc.putter.Put(tc.ctx, job.chunk, job.putterWg)
				if err != nil {
					tc.errC <- err
					return
				}
				// fmt.Printf(" runWorker:Put %d result: %x\n", len(job.chunk), h)
				copy(job.key, h)
				job.parentWg.Done()
			case <-tc.quitC:
				return
