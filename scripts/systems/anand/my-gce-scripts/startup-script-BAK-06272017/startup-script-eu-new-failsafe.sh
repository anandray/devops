...
#Copy CRONJOBS:
sudo su - << EOF
gsutil cp gs://startup_scripts_us/scripts/cron_root-eu /var/spool/cron/root;
...
sudo gsutil cp gs://startup_scripts_us/scripts/httpd_conf-eu.sh /root/scripts/;
...
sudo gsutil cp gs://startup_scripts_us/scripts/hbase/hbasechk-eu.sh /root/scripts;
...
sudo gsutil cp gs://startup_scripts_us/scripts/nagios/startup-script-eu-new-failsafe.sh /root/scripts/;
...
sudo gsutil cp gs://startup_scripts_us/scripts/hbase/hbase-install-centOS-eu.sh /home/anand;
...
/bin/sed -i 's/\*\/1 \* \* \* \* \/usr\/bin\/flock \-w 0 \/var\/run\/startup-script-eu-new-failsafe.lock \/bin\/sh \/root\/scripts\/startup-script-eu-new-failsafe.sh/\#\*\/1 \* \* \* \* \/usr\/bin\/flock \-w 0 \/var\/run\/startup-script-eu-new-failsafe.lock \/bin\/sh \/root\/scripts\/startup-script-eu-new-failsafe.sh/g' /var/spool/cron/root
fi
