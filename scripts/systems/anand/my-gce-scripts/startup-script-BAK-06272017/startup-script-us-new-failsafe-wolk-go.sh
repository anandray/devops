...
#Copy CRONJOBS:
sudo su - << EOF
gsutil cp gs://startup_scripts_us/scripts/cron_root-us-wolk-go /var/spool/cron/root;
...
sudo gsutil cp gs://startup_scripts_us/scripts/startup-script-us-new-failsafe-wolk-go.sh /root/scripts/;
...
sudo gsutil cp gs://startup_scripts_us/scripts/cron.hourly_crosschannel /etc/cron.hourly/crosschannel;
sudo gsutil cp gs://startup_scripts_us/scripts/logrotate.d_crosschannel /etc/logrotate.d/crosschannel;
...
if [ ! -d /var/www/vhosts/www.wolk.com ]; then
        ...
        sudo su - << EOF
        cd /var/www/vhosts/;
        git clone git@github.com:wolktoken/www.wolk.com.git /var/www/vhosts/www.wolk.com;
...
sudo gsutil cp gs://startup_scripts_us/scripts/go/go-crosschannel-install.sh /home/anand;
...
sudo su - << EOF
cd /var/www/vhosts/crosschannel.com/bidder/bin && sh goservice.sh wolk &>> /var/log/wolk.log
EOF
fi

if ! netstat -apn | grep wolk > /dev/null; then
  sudo su - << EOF
...
sed -i 's/\*\/1 \* \* \* \* \/usr\/bin\/flock -w 0 \/var\/run\/startup-script-us-new-failsafe-wolk-go.lock/#\*\/1 \* \* \* \* \/usr\/bin\/flock -w 0 \/var\/run\/startup-script-us-new-failsafe-wolk-go.lock/g' /var/spool/cron/root
