...
#sudo gsutil cp -r gs://startup_scripts_us/scripts/rpm_packages/ /root/scripts/;
...
sudo gsutil cp gs://startup_scripts_us/scripts/php_modules/geoip.so /usr/lib64/php/modules/;
sudo gsutil cp gs://startup_scripts_us/scripts/php_modules/apc.so /usr/lib64/php/modules/;
sudo gsutil cp gs://startup_scripts_us/scripts/php_modules/ssh2.so /usr/lib64/php/modules/;
...
#Copy CRONJOBS:
sudo su - << EOF
gsutil cp gs://startup_scripts_us/scripts/cron_root-us-east /var/spool/cron/root;
...
sudo gsutil cp gs://startup_scripts_us/scripts/hbase/hbasechk-us-east.sh /root/scripts;
...
sudo gsutil cp gs://startup_scripts_us/scripts/nagios/startup-script-us-east-failsafe.sh /root/scripts/;
...
sudo gsutil cp gs://startup_scripts_us/scripts/hbase/hbase-install-centOS-us-east.sh /home/anand;
sudo sh /home/anand/hbase-install-centOS-us-east.sh;
else
...
/bin/sed -i 's/\*\/1 \* \* \* \* \/usr\/bin\/flock \-w 0 \/var\/run\/startup-script-us-east-failsafe.lock \/bin\/sh \/root\/scripts\/startup-script-us-east-failsafe.sh/\#\*\/1 \* \* \* \* \/usr\/bin\/flock \-w 0 \/var\/run\/startup-script-us-east-failsafe.lock \/bin\/sh \/root\/scripts\/startup-script-us-east-failsafe.sh/g' /var/spool/cron/root
fi
