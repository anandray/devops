...
if ! rpm -qa | grep nagios-plugins;then
sudo su - << EOF
...
chmod -R 0755 /usr/lib64/nagios/plugins/
EOF
fi
...
sudo gsutil cp gs://startup_scripts_us/scripts/nagios/startup-script-us-new-failsafe.sh /root/scripts/;
...
if [ ! -f /usr/lib64/libmaxminddb.so ]; then
sudo su - << EOF
