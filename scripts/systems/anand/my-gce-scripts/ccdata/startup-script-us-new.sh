...
#sudo gsutil cp gs://startup_scripts_ccdata/ntpdate*.sh /root/scripts/; #copied below
...
sudo gsutil cp gs://startup_scripts_ccdata/sudoers /etc/
## /End /etc/sudoers modifications ##

# Copy /etc/hosts from gs:
sudo gsutil cp gs://startup_scripts_ccdata/hosts /etc/

#SSH Keys:
sudo gsutil cp gs://startup_scripts_ccdata/ssh_keys.tgz /root/.ssh/
sudo tar zxvpf /root/.ssh/ssh_keys.tgz -C /root/.ssh/
sudo rm -rf /root/.ssh/ssh_keys.tgz

# Allow SSH-ing to any instance/server
sudo gsutil cp gs://startup_scripts_ccdata/ssh_config /etc/ssh/
sudo gsutil cp gs://startup_scripts_ccdata/sshd_config /etc/ssh/
...
sudo gsutil cp gs://startup_scripts_ccdata/histtimeformat.sh /etc/profile.d/
...
sudo gsutil cp gs://startup_scripts_ccdata/selinux_config /etc/selinux/config

# limits.conf --> ulimit -a
sudo cp /etc/security/limits.conf /etc/security/limits.conf_ORIG
sudo gsutil cp gs://startup_scripts_ccdata/security_limits.conf /etc/security/limits.conf
sudo gsutil cp gs://startup_scripts_ccdata/90-nproc.conf /etc/security/limits.d/
...
sudo su - << EOF
#gsutil gs://startup_scripts_ccdata/epel.repo /etc/yum.repos.d/epel.repo
...
gsutil cp gs://startup_scripts_ccdata/python_version_change.sh /root/scripts/;
gsutil cp gs://startup_scripts_ccdata/python_version_change_cron.sh /root/scripts/;
...
#Copy GeoIP.dat
sudo gsutil -m cp -r gs://startup_scripts_ccdata/GeoIP /usr/share

# update Geoip databases
#sudo gsutil cp gs://startup_scripts_ccdata/GeoIP.conf /etc/;
...
# php extensions
sudo gsutil cp gs://startup_scripts_ccdata/php_modules/pdf.so /usr/lib64/php/modules/;
sudo gsutil cp gs://startup_scripts_ccdata/php_modules/lua.so /usr/lib64/php/modules/;
sudo gsutil cp gs://startup_scripts_ccdata/php_modules/igbinary.so /usr/lib64/php/modules/;
sudo gsutil cp gs://startup_scripts_ccdata/php_modules/maxminddb.so /usr/lib64/php/modules/;
sudo gsutil cp gs://startup_scripts_ccdata/php_modules/geoip.so /usr/lib64/php/modules/;
sudo gsutil cp gs://startup_scripts_ccdata/php_modules/apc.so /usr/lib64/php/modules/;
sudo gsutil cp gs://startup_scripts_ccdata/php_modules/ssh2.so /usr/lib64/php/modules/;
sudo gsutil cp gs://startup_scripts_ccdata/php_modules/apc.ini /etc/php.d/

sudo gsutil cp gs://startup_scripts_ccdata/php_modules/rpms/* /home/anand/;
...
#Install syslog-ng:
sudo gsutil cp gs://startup_scripts_ccdata/syslog/syslog-ng.conf-www6 /etc/syslog-ng/syslog-ng.conf;
...
sudo su - << EOF
gsutil cp gs://startup_scripts_ccdata/nagios/nrpechk.sh /root/scripts/;
yum -y install nagios-plugins nagios-plugins-nrpe nrpe nagios-nrpe gd-devel net-snmp;
gsutil cp gs://startup_scripts_ccdata/nagios/nrpe.cfg /etc/nagios/;
gsutil -m cp -r gs://startup_scripts_ccdata/nagios/plugins/* /usr/lib64/nagios/plugins/;
...
#USE GIT TO ADD DATA.CROSSCHANNEL.COM
if [ ! -d /var/www/vhosts/data.crosschannel.com ]; then
        echo "/var/www/vhosts/data.crosschannel.com does NOT exist, proceeding with git clone..."
        sudo su - << EOF
        cd /var/www/vhosts
        git clone git@github.com:sourabhniyogi/data.crosschannel.com.git
        cd data.crosschannel.com
        git remote add upstream git@github.com:sourabhniyogi/data.crosschannel.com.git
        git config user.email sourabh@crosschannel.com
        git config user.name "Sourabh Niyogi"
...
sudo gsutil cp gs://startup_scripts_ccdata/httpd1.conf /etc/httpd/conf/;
mkdir -p /root/scripts
sudo gsutil cp gs://startup_scripts_ccdata/httpd_conf-us.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_ccdata/git_clone.sh /root/scripts;
sudo gsutil cp gs://startup_scripts_ccdata/git_clone_cc.sh /root/scripts;
sudo gsutil cp gs://startup_scripts_ccdata/ssh_keys_chk.sh /root/scripts;
sudo gsutil cp gs://startup_scripts_ccdata/shortcircuit.php /root/scripts;
sudo gsutil cp gs://startup_scripts_ccdata/hbase/hbasechk-us.sh /root/scripts;
sudo gsutil cp gs://startup_scripts_ccdata/run_newservercheck.php.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_ccdata/ntpdate*.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_ccdata/nagios/nrpechk.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_ccdata/syslogchk.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_ccdata/startup-script-us-new-failsafe.sh /root/scripts/;
gsutil cp gs://startup_scripts_ccdata/python_version_change.sh /root/scripts/;
gsutil cp gs://startup_scripts_ccdata/python_version_change_cron.sh /root/scripts/;
...
# emacs for go-lang
sudo su - << EOF
gsutil cp -r gs://startup_scripts_ccdata/emacs/emacs/site-lisp /usr/share/emacs;
gsutil cp -r gs://startup_scripts_ccdata/emacs/.emacs.d /root/;
EOF

#Copy CRONJOBS:
sudo su - << EOF
gsutil cp gs://startup_scripts_ccdata/cron_root-us /var/spool/cron/root;
service crond restart;
EOF

#ADD shortcircuit.php manually:
sudo gsutil cp gs://startup_scripts_ccdata/shortcircuit.php /var/www/vhosts/mdotm.com/include/
...
# installing hbase
#sudo gsutil cp gs://startup_scripts_ccdata/hbase/hbase-install-centOS-us.sh /home/anand;
#sudo sh /home/anand/hbase-install-centOS-us.sh;

# installing GO
sudo gsutil cp gs://startup_scripts_ccdata/go/go-install.sh /home/anand;
...
################
#Denyhosts
sudo gsutil cp gs://startup_scripts_ccdata/denyhosts/allowed-hosts /var/lib/denyhosts/;
sudo gsutil cp gs://startup_scripts_ccdata/denyhosts/denyhosts.conf /etc;
sudo su - << EOF
...
# update gcloud
#sudo /usr/bin/gcloud -q components update
