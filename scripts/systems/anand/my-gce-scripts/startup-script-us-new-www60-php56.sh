...
sudo gsutil -m cp gs://startup_scripts_us/scripts/sudoers_with_apache /etc/sudoers
...
sudo gsutil cp gs://startup_scripts_us/scripts/php_modules/40-igbinary.ini /etc/php.d/;
sudo gsutil cp gs://startup_scripts_us/scripts/php_modules/40-igbinary.ini /etc/php-zts.d/;
sudo gsutil cp gs://startup_scripts_us/scripts/php_modules/40-geoip.ini /etc/php.d/;
sudo gsutil cp gs://startup_scripts_us/scripts/php_modules/40-geoip.ini /etc/php-zts.d/;
...
sudo gsutil -m cp gs://startup_scripts_us/scripts/admin_htaccess/.htaccess /var/www/vhosts/mdotm.com/httpdocs/.htaccess
...
sudo gsutil -m cp gs://startup_scripts_us/scripts/startup-script-us-new-failsafe-php56u.sh /root/scripts/;
...
sudo gsutil cp gs://startup_scripts_us/scripts/bashrc /root/.bashrc;
sudo chmod -R +x /root/scripts
sudo sh /root/scripts/httpd_conf-us.sh

# creating dirs in /var/www to allow apache to upload files to the UI
sudo mkdir -p /var/www/.config/gcloud /var/www/.gsutil
sudo chgrp -R apache /var/www/.config/gcloud /var/www/.gsutil
sudo chmod 0775 /var/www/.config/gcloud /var/www/.gsutil

#Copy CRONJOBS:
sudo su - << EOF
gsutil -m cp gs://startup_scripts_us/scripts/cron_root-us-php56u /var/spool/cron/root;
