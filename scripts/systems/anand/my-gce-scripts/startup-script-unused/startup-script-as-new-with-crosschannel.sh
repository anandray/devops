...
sudo gsutil cp gs://startup_scripts_us/scripts/startup-script-as-new-failsafe.sh /root/scripts/;
...
# running go command
#cd /var/www/vhosts/crosschannel.com/bidder/bin && git fetch upstream && git merge upstream/master && php goservice.php 2> /var/log/git.err  > /var/log/git.log
sudo su - << EOF
