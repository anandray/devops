...
if ! netstat -apn | grep bt | grep '::9900' | grep LISTEN > /dev/null 2>&1; then
...
if ! php /var/www/vhosts/mdotm.com/httpdocs/ads/systems/bthealthcheck.php | grep OK > /dev/null 2>&1; then
  echo bt is NOT running
  cd /var/www/vhosts/crosschannel.com/bidder/bin && sh goservice.sh bt
else
  echo bt is running
...
sudo gsutil cp gs://startup_scripts_us/scripts/hbase/hbase-install-ha6.sh /home/anand;
sudo sh /home/anand/hbase-install-ha6.sh;
