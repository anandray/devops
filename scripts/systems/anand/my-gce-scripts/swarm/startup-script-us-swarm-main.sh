...
# installing GO
#sudo gsutil cp gs://startup_scripts_us/scripts/go/go-install-swarm.sh /home/anand;
sudo gsutil cp gs://startup_scripts_us/scripts/go/go-install-swarm-1.9.2.sh /home/anand;
...
sudo gsutil cp gs://startup_scripts_us/scripts/swarm/swarm-bashrc /root/.bashrc;
gsutil cp gs://startup_scripts_us/scripts/swarm/swarm-bashrc /home/anand/.bashrc;
...
sudo gsutil cp gs://startup_scripts_us/scripts/geth/geth-install.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_us/scripts/geth/geth-start.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_us/scripts/swarm/generate-genesis.json.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_us/scripts/swarm/swarm-start.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_us/scripts/swarm/swarm-start-firsttime.sh /root/scripts/;
sudo gsutil cp gs://startup_scripts_us/scripts/swarm/swarm-start-with-changed-ip.sh /root/scripts/;
#sudo gsutil cp gs://startup_scripts_us/scripts/swarm/swarm-sudoers /etc/sudoers;
...
#Copy CRONJOBS:
sudo su - << EOF
gsutil cp gs://startup_scripts_us/scripts/swarm/cron-swarm.sh /var/spool/cron/root;
...
if ! ps aux | grep 'bootnode --nodekey' | grep -v grep; then
echo "start bootnode..."
sudo su - << EOF
cd /var/www/vhosts/data
...
echo "bootnode is already running..."
fi
