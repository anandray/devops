...
/sbin/sysctl -p;
ssh `hostname` sh /root/scripts/hbasechk.sh;
sed -i '/hbasechk.sh/d' /var/spool/cron/root;
echo '*/1 * * * * ssh `hostname` /bin/sh /root/scripts/hbasechk.sh > /var/log/hbase.log 2>&1' >> /var/spool/cron/root;
EOF
