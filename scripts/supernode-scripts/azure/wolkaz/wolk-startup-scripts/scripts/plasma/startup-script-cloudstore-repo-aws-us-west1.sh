...
sudo aws s3 cp s3://wolk-startup-scripts/scripts/swarm/swarm-sudoers-aws /etc/sudoers;
...
AWSDynamoCredentials="/root/go/src/github.com/wolkdb/cloudstore/wolk/cloud/credentials/credentials"
...
sed -i 's|"_AWSDynamoCredentials"|"'$AWSDynamoCredentials'"|g' /root/go/src/github.com/wolkdb/cloudstore/wolk.toml # using '|' instead of '/' because of error: "sed: -e expression #1, char 35: unknown option to `s'"
