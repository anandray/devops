...
sudo -m gsutil -m cp -r gs://startup_scripts_us/scripts/rpm_packages/ /root/scripts/;
...
#Copy CRONJOBS:
sudo su - << EOF
gsutil -m cp gs://startup_scripts_us/scripts/cron_root-as /var/spool/cron/root;
...
sudo gsutil -m cp gs://startup_scripts_us/scripts/httpd_conf-as.sh /root/scripts/;
...
sudo gsutil -m cp gs://startup_scripts_us/scripts/hbase/hbasechk-as.sh /root/scripts;
...
sudo gsutil -m cp gs://startup_scripts_us/scripts/nagios/startup-script-as-new-failsafe.sh /root/scripts/;
...
sudo sh /root/scripts/httpd_conf-as.sh
...
sudo gsutil -m cp gs://startup_scripts_us/scripts/hbase/hbase-install-centOS-as.sh /home/anand;
sudo sh /home/anand/hbase-install-centOS-as.sh;
else
...
/bin/sed -i 's/\*\/1 \* \* \* \* \/usr\/bin\/flock \-w 0 \/var\/run\/startup-script-as-new-failsafe.lock \/bin\/sh \/root\/scripts\/startup-script-as-new-failsafe.sh/\#\*\/1 \* \* \* \* \/usr\/bin\/flock \-w 0 \/var\/run\/startup-script-as-new-failsafe.lock \/bin\/sh \/root\/scripts\/startup-script-as-new-failsafe.sh/g' /var/spool/cron/root
fi
