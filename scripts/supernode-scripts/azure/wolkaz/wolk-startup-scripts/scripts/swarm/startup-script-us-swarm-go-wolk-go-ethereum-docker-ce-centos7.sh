...
sudo yum -y install gcc make emacs ntpdate telnet screen git denyhosts python-argparse *whois openssl openssl-devel java-1.8.0-openjdk-devel python27-pip lynx net-tools mlocate wget sqlite-devel;
...
# GIT CLONE GO-WOLK
if [ ! -d /var/www/vhosts/go-wolk ]; then
        echo "/var/www/vhosts/go-wolk does NOT exist, proceeding with git clone..."
        sudo su - << EOF
	mkdir -p /var/www/vhosts;
        cd /var/www/vhosts/;
	git clone -v --recurse-submodules -b phobos git@github.com:wolkdb/go-wolk.git
	cd go-wolk
	git remote -v add upstream git@github.com:wolkdb/go-wolk.git
	git config user.email anand@wolk.com
	git config user.name "anand ray"
	git config --global core.filemode true
	git config core.filemode true
        git fetch upstream;
        git merge upstream/phobos;
EOF
fi

# GIT CLONE GO-ETHERUEM
if [ ! -d /var/www/vhosts/go-ethereum ]; then
        echo "/var/www/vhosts/go-ethereum does NOT exist, proceeding with git clone..."
        sudo su - << EOF
        mkdir -p /var/www/vhosts;
        cd /var/www/vhosts/;
        git clone -v --recurse-submodules git@github.com:ethereum/go-ethereum.git
        cd go-wolk
        git remote -v add upstream git@github.com:ethereum/go-ethereum.git
        git config user.email anand@wolk.com
        git config user.name "anand ray"
        git config --global core.filemode true
        git config core.filemode true
...
# installing GO
sudo gsutil -m cp gs://startup_scripts_us/scripts/go/go-install-swarm-1.10.1.sh /home/anand;
sudo sh /home/anand/go-install-swarm-1.10.1.sh;
...
sudo gsutil -m cp gs://startup_scripts_us/scripts/swarm/swarm-bashrc-centos7 /root/.bashrc;
gsutil -m cp gs://startup_scripts_us/scripts/swarm/swarm-bashrc-centos7 /home/anand/.bashrc;
...
## GIT CLONE NETSTATS.WOLK.COM
#if [ ! -d /var/www/vhosts/netstats.wolk.com ]; then
#        echo "/var/www/vhosts/netstats.wolk.com does NOT exist, proceeding with git clone..."
#        sudo su - << EOF
#        mkdir -p /var/www/vhosts;
#        cd /var/www/vhosts/;
#        git clone -v git@github.com:wolkdb/netstats.wolk.com.git
#        cd netstats.wolk.com
#        git remote -v add upstream git@github.com:wolkdb/netstats.wolk.com.git
...
## build geth + swarm docker images and run
#sudo su - << EOF
#cd /var/www/vhosts/netstats.wolk.com/docker/wolkmain
#chmod +x /var/www/vhosts/netstats.wolk.com/docker/wolkmain/scripts/*
#chmod +x /var/www/vhosts/netstats.wolk.com/docker/wolknode/scripts/*
##docker build -t wolkmain .
#EOF

# Compiling geth in go-wolk
