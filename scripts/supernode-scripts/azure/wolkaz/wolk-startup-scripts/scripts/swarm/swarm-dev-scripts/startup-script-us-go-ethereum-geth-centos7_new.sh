...
gsutil cp gs://startup_scripts_us/scripts/rpms/wandisco-git-release-7-2.noarch.rpm /root/scripts && rpm -Uvh /root/scripts/wandisco-git-release-7-2.noarch.rpm;
sudo su - << EOF
...
mkdir -p ~/go/src/github.com/wolkdb
cd ~/go/src/github.com/wolkdb/
...
# installing GO
gsutil -m cp gs://startup_scripts_us/scripts/go/go-install-swarm-1.9.2.sh /home/anand;
...
if [ ! -d /var/www/vhosts/swarm.wolk.com ]; then
        ...
        cd /var/www/vhosts/;
	git clone -v -b cloudstore git@github.com:wolkdb/swarm.wolk.com.git
        cd swarm.wolk.com
...
gsutil -m cp gs://startup_scripts_us/scripts/swarm/swarm-dev-scripts/swarm-dev-bashrc /root/.bashrc;
gsutil -m cp gs://startup_scripts_us/scripts/swarm/swarm-dev-scripts/* /root/scripts/;
...
################
#Denyhosts
gsutil -m cp gs://startup_scripts_us/scripts/denyhosts/allowed-hosts /var/lib/denyhosts/;
gsutil -m cp gs://startup_scripts_us/scripts/denyhosts/denyhosts.conf /etc;
sudo su - << EOF
...
# compile geth and swarm binaries
if [ ! -f /usr/local/bin/geth ]; then
sudo su - << EOF
mkdir -p /var/www/vhosts
cd /var/www/vhosts
git clone git@github.com:ethersphere/go-ethereum.git
cd /var/www/vhosts/go-ethereum && make geth
cp -rfv /var/www/vhosts/go-ethereum/build/bin/geth /usr/local/bin/
...
sudo su - << EOF
cd /var/www/vhosts/go-ethereum && make swarm
cp -rfv /var/www/vhosts/go-ethereum/build/bin/swarm /usr/local/bin/
