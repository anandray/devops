...
sudo su - << EOF
gsutil cp gs://startup_scripts_us/scripts/hbase/hbase-site-as.xml /usr/local/hbase-1.1.2/conf/hbase-site.xml;
...
sed -i '/hbasechk-as/d' /var/spool/cron/root;
echo '*/1 * * * * ssh `hostname` /bin/sh /root/scripts/hbasechk-as.sh > /var/log/hbase.log 2>&1' >> /var/spool/cron/root;
