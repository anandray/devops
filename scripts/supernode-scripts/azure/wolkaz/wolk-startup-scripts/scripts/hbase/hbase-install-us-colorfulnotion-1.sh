...
sudo su - << EOF
gsutil cp gs://startup-scripts-colorfulnotion/scripts/hbase/hbase-site-us-colorfulnotion.xml /usr/local/hbase-1.1.2/conf/hbase-site.xml;
gsutil cp gs://startup-scripts-colorfulnotion/scripts/hbase/hbase-env.sh /usr/local/hbase-1.1.2/conf/;
sed -i '/JAVA_HOME/d' /root/.bashrc;
echo 'export JAVA_HOME=/usr/lib/jvm/jre-1.8.0-openjdk.x86_64/' >> /root/.bashrc;
export JAVA_HOME=/usr/lib/jvm/jre-1.8.0-openjdk.x86_64/;
mkdir -p /root/scripts/;
gsutil cp gs://startup-scripts-colorfulnotion/scripts/roam_scripts/hbasechk-us-colorfulnotion.sh /root/scripts;
...
/sbin/sysctl -p;
sh /root/scripts/hbasechk-us-colorfulnotion.sh;
sed -i '/hbasechk-us/d' /var/spool/cron/root;
echo '*/1 * * * * ssh `hostname` /bin/sh /root/scripts/hbasechk-us-colorfulnotion.sh > /var/log/hbase.log 2>&1' >> /var/spool/cron/root;
