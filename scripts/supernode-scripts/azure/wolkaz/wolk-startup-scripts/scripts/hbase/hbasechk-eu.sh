...
#echo hbase is NOT running
#pkill -9 java;
##This time delete /usr/local/hbase-1.1.2 directory so Hbase is re-installed in the next step
#rm -rf /usr/local/hbase-1.1.2;
#fi
...
sudo gsutil cp gs://startup_scripts_us/scripts/hbase/hbase-install-centOS-eu.sh /root/scripts;
sudo sh /root/scripts/hbase-install-centOS-eu.sh;
fi
