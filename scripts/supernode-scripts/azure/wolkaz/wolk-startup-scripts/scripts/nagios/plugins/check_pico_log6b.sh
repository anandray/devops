...
#KILL=`kill -9 $(ssh -q log6b ps aux | grep pico.php | grep -v grep | awk '{print$2}')`
#KILL=$(kill -9 $(ssh -q log6b ps aux | grep pico.php | grep -v grep | awk '{print$2}')) > /dev/null
#PICO_CHECK=$(ssh -q log6b ls -lt /var/log/pico.log  | grep "`date +%b\ \ %-d\ %H:`" | wc -l) #sigle digit date (from 1st thru 9th)
TIMESTAMP1=`date +%b\ %-d\ %H:`
TIMESTAMP2=`date +%b\ \ %-d\ %H:`
